...
(function(){
  ...
  const formatNum = (x, digits=8) => {
    ...
    return Number(x).toPrecision(10).replace(/\.0+$/,'').replace(/(\..*?)0+$/,'$1');
  };
  ...
  function buildCumListFromCounts(counts, symbols){
    let total = 0;
    for (const ch of symbols){ total += counts.get(ch) || 0; }
    let cum = 0;
    const cumList = [];
  ...
  function buildAdaptiveEncode(text, symbols){
    ...
    for (const ch of symbols){ counts.set(ch, 1); } // Laplace smoothing
    const cumLists = [];
    const steps = [];
    let low = 0, high = 1;
    for (let i=0;i<text.length;i++){
      ...
      steps.push({i, ch, prevLow: low, prevHigh: high, low: nlow, high: nhigh, range: nhigh-nlow, cumLow: row.cumLow, cumHigh: row.cumHigh});
      low = nlow; high = nhigh;
      counts.set(ch, (counts.get(ch)||0)+1);
    }
    return { steps, cumLists, initial: buildCumListFromCounts(new Map(Array.from(symbols, s=>[s,1])), symbols) };
  }

  function buildAdaptiveDecode(codeValue, symbols, targetLen){
    const counts = new Map();
    for (const ch of symbols){ counts.set(ch, 1); }
    const cumLists = [];
    const steps = [];
    let low = 0, high = 1;
    const eps = 1e-12;
    for (let i=0;i<targetLen;i++){
      ...
      let found = null;
      for (const row of cumList){ if (t >= row.cumLow - eps && t < row.cumHigh - eps){ found = row; break; } }
      if (!found) break;
      const nlow = low + range * found.cumLow;
      const nhigh = low + range * found.cumHigh;
      steps.push({i, ch: found.ch, prevLow: low, prevHigh: high, low: nlow, high: nhigh, range: nhigh-nlow, cumLow: found.cumLow, cumHigh: found.cumHigh});
      low = nlow; high = nhigh;
  ...
  function buildSteps(text, cumMap){
    ...
    for (let i=0;i<text.length;i++){
      ...
      steps.push({i, ch, prevLow: low, prevHigh: high, low: nlow, high: nhigh, range: nhigh - nlow, cumLow: c.low, cumHigh: c.high});
      low = nlow; high = nhigh;
    }
    return steps;
  }
  ...
  function startTween(newIndex){
    ...
    state.anim.fromLow = from.low; state.anim.fromHigh = from.high;
    state.anim.toLow = to.low; state.anim.toHigh = to.high;
  ...
  function buildDecodeSteps(codeValue, cumList, targetLen){
    ...
    for (let i=0;i<targetLen;i++){
      ...
      if (!found){
        // numeric fallbacks
        for (const row of cumList){
          if (t >= row.cumLow - 1e-9 && t <= row.cumHigh + 1e-9){ found = row; break; }
        }
      }
      if (!found) break;
      const nlow = low + range * found.cumLow;
  ...
  function renderModelTable(cumList){
    ...
    for (const row of cumList){
      ...
      tr.appendChild(tdCh); tr.appendChild(tdCnt); tr.appendChild(tdP); tr.appendChild(tdCum);
      tb.appendChild(tr);
    }
  }
  ...
  function drawGlobal(ctx, width, height, cumList, currLow, currHigh, highlightCh){
    ...
    for (const row of cumList){
      ...
      ctx.fillStyle = row.ch===highlightCh ? lighten(colorForSymbol(row.ch), 10) : colorForSymbol(row.ch);
  ...
  function rebuildZoomCumList(baseCumList, low, high){
    ...
    return baseCumList.map(r=>({ch:r.ch, p:r.p, cumLow:r.cumLow, cumHigh:r.cumHigh, count:r.count}));
  }
  ...
  function updateRecFrame(){
    ...
    const k = state.stepIdx, N = state.steps.length;
    ...
    if (state.mode === 'encode'){
      if (N>0){
        ...
        ctx.fillText(`low=${formatNum(last.low,6)}  high=${formatNum(last.high,6)}`, 8, state.rec.canvas.height - 18);
      }
    } else {
      const prefix = state.steps.slice(0, k).map(s=>s.ch).join('');
  ...
  function render(){
    ...
    const currLow = disp.low; const currHigh = disp.high;
    ...
    const gz = el.barGlobal.getBoundingClientRect();
    const renderCum = state.modelMode === 'adaptive' ? (state.cumLists[k] || state.cumLists[state.cumLists.length-1] || state.cumList) : state.cumList;
    ...
    if (state.mode === 'decode' && state.codeValue!=null){
      ...
      if (range > 0){ tVal = (state.codeValue - currLow) / range; }
    }
  ...
  el.buildModel.addEventListener('click', ()=>{
    try{
      ...
      if (modelType === 'adaptive'){
        ...
        if (!symbols.length){ alert('无法从输入构建字母表'); return; }
        ...
        if (model.alphabetSize===0){ alert('无法从输入构建模型'); return; }
  ...
  el.buildDecode.addEventListener('click', ()=>{
    try{
      if (state.modelMode === 'static'){
        if (!state.cumList.length){ alert('请先生成模型'); return; }
      } else {
        if (!state.symbols || !state.symbols.length){ alert('请先通过输入文本生成模型以确定字母表'); return; }
      }
      stopPlaying();
      const v = parseCodeValue(el.decodeValue.value);
      if (v==null){ alert('请输入有效的码值：二进制 0.01... 或十进制 0.xxxx'); return; }
      let L = parseInt(el.decodeLen.value, 10);
      if (!(L>0)){
        if (state.text && state.text.length>0) L = state.text.length; else { alert('请填写解码步数'); return; }
      }
      let steps, cumLists;
      if (state.modelMode === 'adaptive'){
        const out = buildAdaptiveDecode(v, state.symbols, L);
        steps = out.steps; cumLists = out.cumLists;
      } else {
      ...
      state.targetLen = L;
      state.steps = steps;
      if (state.modelMode === 'adaptive'){ state.cumLists = cumLists; }
      state.stepIdx = 0;
      render();
    }catch(err){
      console.error(err);
  ...
  el.fillFromEncode.addEventListener('click', ()=>{
    if (state.lastEncode && state.lastEncode.value!=null){
      ...
      if (state.text && state.text.length>0){ el.decodeLen.value = String(state.text.length); }
    } else {
      alert('请先完成一次编码以获取码值');
    }
  });
  ...
  function stopRecording(){
    ...
    try{ state.rec.recorder && state.rec.recorder.stop(); }catch(_){/* ignore */}
    state.rec.recorder = null;
    el.recBtn.textContent = '开始录制(WebM)';
  }

  el.recBtn.addEventListener('click', ()=>{
    if (!state.rec.active) startRecording(); else stopRecording();
  });
  ...
  el.snapBtn.addEventListener('click', ()=>{
    ...
    ctx.fillStyle = '#0f1115'; ctx.fillRect(0,0,tmp.width,tmp.height);
    try{ ctx.drawImage(el.barGlobal, 0, 10, tmp.width, gz.height); }catch(_){ }
    try{ ctx.drawImage(el.barZoom, 0, 20+gz.height, tmp.width, zz.height); }catch(_){ }
    ctx.fillStyle = '#b8c0cc';
    ctx.font = '14px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial';
    const k = state.stepIdx, N = state.steps.length;
    const mode = state.mode === 'encode' ? '编码' : '解码';
    ...
    const a = document.createElement('a');
    a.href = url; a.download = `arith-${state.mode}-${Date.now()}.png`; a.click();
    setTimeout(()=>URL.revokeObjectURL(url), 1000);
  });
  ...
  if (el.exportGifBtn){
    el.exportGifBtn.addEventListener('click', async ()=>{
      if (!window.gifshot){ alert('未加载 gifshot 库'); return; }
      ...
      gifshot.createGIF({
        ...
        if (!obj.error){
          const a = document.createElement('a');
          a.href = obj.image; a.download = `arith-${state.mode}-${Date.now()}.gif`; a.click();
        } else {
          alert('生成GIF失败');
        }
      });
  ...
  if (el.exportPptBtn){
    el.exportPptBtn.addEventListener('click', async ()=>{
      if (typeof PptxGenJS === 'undefined'){ alert('未加载 PptxGenJS'); return; }
      ...
      if (state.cumList && state.cumList.length){
        ...
        state.cumList.forEach(r=>{
          rows.push([ r.ch===' ' ? '␠' : r.ch, String(r.count), formatPct(r.p), `[${formatNum(r.cumLow,6)}, ${formatNum(r.cumHigh,6)})` ]);
        });
  ...
  function hideTooltip(){ if (el.tooltip) el.tooltip.style.display = 'none'; }
  ...
  function bindCanvasTooltip(canvas){
    ...
    canvas.addEventListener('mousemove', (ev)=>{
      ...
      let row = null;
      for (const r of cumList){ if (t >= r.cumLow && t < r.cumHigh){ row = r; break; } }
      if (!row) return hideTooltip();
      const html = `<div><b>符号</b>: ${row.ch===' ' ? '␠' : row.ch}</div>`+
...
`);
      ...
      for (let i=0;i<N;i++){
        ...
        out += `  S${i} --> S${i+1}[第${i+1}步: t=${formatNum(t,6)} 命中 '${esc(s.ch)}'\\n区间→[${formatNum(s.low,6)}, ${formatNum(s.high,6)})]` + '\n';
      }
      if (steps.length > N){ out += `  S${N} --> E[... 共 ${steps.length} 步]\n`; }
      return out;
    } else if (kind === 'pie'){
...
`);
  ...
  if (el.copyMermaid){
    el.copyMermaid.addEventListener('click', async ()=>{
      const txt = state.mermaidText || getMermaidText((el.mermaidType && el.mermaidType.value) || 'encode');
      try{
  ...
  async function saveMermaidAsPNG(){
    ...
    if (!svgEl){ alert('请先渲染 Mermaid 图'); return; }
    ...
    try{
      ...
      const a = document.createElement('a'); a.href = png; a.download = `mermaid-${Date.now()}.png`; a.click();
      setTimeout(()=>URL.revokeObjectURL(url), 1000);
    }catch(err){
  ...
  async function exportAllFrames(){
    ...
    for (let i=0;i<=N;i++){
      ...
      const url = tmp.toDataURL('image/png');
      const a = document.createElement('a');
      a.href = url; a.download = `arith-${state.mode}-${pad(i,widthDigits)}.png`; a.click();
      await new Promise(r=>setTimeout(r, 30));
    }
  }
