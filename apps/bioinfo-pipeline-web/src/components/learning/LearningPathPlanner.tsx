...
export default function LearningPathPlanner() {
  ...
  return (
    <div className="space-y-8">
      ...
      <Card elevation="md" className="overflow-hidden border-0 bg-gradient-to-br from-white to-zinc-50 dark:from-zinc-900 dark:to-black">
        {/* Header & Progress */}
                    ...
                    {step.resources && step.resources.length > 0 && (
                       <div className="mt-3 flex flex-wrap gap-3">
                          {step.resources.map((res) => (
                             ...
                             >
                                {res.kind === 'external' ? <ExternalLink className="h-3 w-3" /> : <BookOpen className="h-3 w-3" />}
                                {res.title}
                             </a>
                          ))}
                       </div>
