...
export default function StageOverviewCards({ pipeline }: { pipeline: Pipeline }) {
  ...
  const progress = asNumber(findMetric(stagesById.get('deg'), 'progress')) ??
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {cards.slice(0, 8).map((card) => (
        <KpiCard key={card.title} title={card.title} value={card.value} unit={card.unit} footnote={card.footnote} />
      ))}
    </div>
  );
}
