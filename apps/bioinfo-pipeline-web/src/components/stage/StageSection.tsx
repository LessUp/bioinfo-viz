...
export default function StageSection({ pipeline, stage }: { pipeline: Pipeline; stage: Stage }) {
  return (
    <section
      id={`stage-${stage.id}`}
      className="scroll-mt-20 rounded-lg border bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"
    >
      ...
      {stage.metrics.length > 0 ? (
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {stage.metrics.slice(0, 4).map((m) => (
            <KpiCard key={m.key} title={m.label} value={m.value} unit={m.unit} />
          ))}
        </div>
      ) : null}
