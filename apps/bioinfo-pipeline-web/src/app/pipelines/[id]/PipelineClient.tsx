      ...
      <header className="sticky top-0 z-10 border-b border-zinc-200/70 bg-white/80 backdrop-blur dark:border-zinc-800 dark:bg-black/70">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <div className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100">{pipeline.profile.name}</div>
            <div className="text-xs text-zinc-500 dark:text-zinc-400">项目：{pipeline.project} · ID：{pipeline.id}</div>
          </div>
          <div className="flex items-center gap-3">
            <Badge tone="accent">{pipeline.profile.difficulty === 'beginner' ? '入门' : pipeline.profile.difficulty === 'intermediate' ? '进阶' : '高级'}</Badge>
            <button
      ...
      <main className="mx-auto max-w-6xl px-6 pt-6">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[2fr_1fr]">
          <Card elevation="md">
            ...
            <p className="mt-2 text-sm leading-relaxed text-zinc-600 dark:text-zinc-300">{pipeline.summary}</p>
            <div className="mt-4">
              <div className="text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">关键知识点</div>
          ...
          <Card elevation="md">
            ...
            <div className="mt-4">
              <div className="text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">适合人群</div>
            ...
            <div className="mt-4">
              <div className="text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">示例样本</div>
        ...
        {pipeline.resources.length > 0 ? (
          <section className="mt-10">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-500 dark:text-zinc-400">将 Docs & Slides 融入教学流程</span>
            </div>
