...
export function DAGView() {
  ...
  const nodes = useMemo(() => {
    ...
    const counts: Record<Step['phase'], number> = {
      Preprocess: 0,
      VariantCalling: 0,
      JointGenotyping: 0,
      Filtering: 0,
    ...
    if (!groupScatter && !groupSubWorkflows) {
      ...
      return run.steps.map((s) => {
        ...
        const n = g.node(s.id)
        return makeStyledNode(s.id, s.label, s.status, hidden, isSelected, isNeighbor, {
          x: n?.x ?? 0,
          y: n?.y ?? 0,
        })
      })
    ...
    const groupOf = (s: Step) => {
      ...
      if (groupSubWorkflows && s.fqname && s.fqname.includes('.'))
        return s.fqname.split('.').slice(0, -1).join('.') + '::{group}'
      return s.id
    }
    ...
    for (const [g, members] of groupMembers.entries()) {
      ...
      const isNeighbor = selectedId
        ? members.some((m) => upstream.has(m.id) || downstream.has(m.id))
        : false
      const hidden = !visibleGroups.has(g)
      const label =
        members[0]?.label + (members.some((m) => m.scatterIndex !== undefined) ? ' [scatter]' : '')
      nodes.push(
        makeStyledNode(g, label, members[0]?.status || 'Queued', hidden, isSelected, isNeighbor)
      )
    }
  ...
  const edges = useMemo(() => {
    ...
    for (const e of run.edges) {
      ...
      const highlight = selectedId
        ? groupOfId(selectedId) === gs || groupOfId(selectedId) === gt
        : false
      out.push({
        id: `e-${out.length}`,
        source: gs,
        target: gt,
        style: highlight ? { stroke: '#111827', strokeWidth: 2 } : undefined,
      })
    }
    return out
  }, [run, filteredSteps, selectedId, groupScatter, groupSubWorkflows])
  ...
  if (!run)
    return <div className="h-full flex items-center justify-center text-gray-500">暂无数据</div>
