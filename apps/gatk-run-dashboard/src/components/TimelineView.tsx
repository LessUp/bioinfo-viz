    ...
    series: [
      ...
      },
      {
        ...
        data: durations.map((v, i) => ({
          value: v,
          itemStyle: { color: colorFor(viewRows[i].status) },
        })),
        label: {
          show: true,
          position: 'right',
      ...
      {shouldWindow && (
        <div className="flex items-center gap-2 px-2 py-1 text-sm">
          ...
          <input
            ...
            value={start}
            onChange={(e) => setWinStart(Number(e.target.value) || 0)}
          />
          <span>大小</span>
          <input
            className="w-20 rounded border px-2 py-1 bg-transparent"
            ...
            max={totalRows}
            value={size}
            onChange={(e) => setWinSize(Number(e.target.value) || 300)}
          />
