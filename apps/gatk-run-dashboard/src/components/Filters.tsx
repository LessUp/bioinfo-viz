  ...
  return (
    <div className="space-y-3">
      ...
      <label className="flex items-center gap-2 text-sm">
        <input
          type="checkbox"
          checked={groupScatter}
          onChange={(e) => setGroupScatter(e.target.checked)}
        />
        Scatter 分组折叠
      </label>
      <label className="flex items-center gap-2 text-sm">
        <input
          type="checkbox"
          checked={groupSubWorkflows}
          onChange={(e) => setGroupSubWorkflows(e.target.checked)}
        />
        子工作流分组折叠
      </label>
      <p className="text-xs text-gray-500">
        更多筛选项（阶段、耗时、重试次数等）可在后续迭代中补充。
      </p>
    </div>
  )
}
