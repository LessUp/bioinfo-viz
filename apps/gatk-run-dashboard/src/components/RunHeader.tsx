...
export function RunHeader() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('gw_baseUrl', baseUrl)
    } catch {}
  }, [baseUrl])
  useEffect(() => {
    try {
      localStorage.setItem('gw_workflowId', workflowId)
    } catch {}
  }, [workflowId])
  useEffect(() => {
    try {
      localStorage.setItem('gw_useProxy', useProxy ? '1' : '0')
    } catch {}
  }, [useProxy])
  useEffect(() => {
    try {
      localStorage.setItem('gw_autoRefresh', autoRefresh ? '1' : '0')
    } catch {}
  }, [autoRefresh])
  useEffect(() => {
    try {
      localStorage.setItem('gw_autoRefreshInterval', String(autoRefreshInterval))
    } catch {}
  }, [autoRefreshInterval])
  useEffect(() => {
    try {
      localStorage.setItem('gw_authEnabled', authEnabled ? '1' : '0')
    } catch {}
  }, [authEnabled])
  useEffect(() => {
    try {
      localStorage.setItem('gw_authHeaderKey', authHeaderKey)
    } catch {}
  }, [authHeaderKey])
  useEffect(() => {
    try {
      localStorage.setItem('gw_authHeaderValue', authHeaderValue)
    } catch {}
  }, [authHeaderValue])
  ...
  return (
    <header className="border-b border-gray-200 dark:border-gray-800 px-4 py-3 flex items-center gap-4">
      ...
      <div className="text-sm text-gray-500">
        {run?.pipeline ?? 'Germline'} · 状态：{run?.status ?? '—'}
      </div>
      <div className="flex items-center gap-2 text-sm">
        ...
        <input
          className="w-36 rounded border px-2 py-1 bg-transparent"
          placeholder="Header Key"
          value={authHeaderKey}
          onChange={(e) => setAuthHeaderKey(e.target.value)}
        />
        <input
          className="w-52 rounded border px-2 py-1 bg-transparent"
      ...
      <div className="ml-auto flex items-center gap-3 text-sm">
        <span className="px-2 py-1 rounded bg-green-100 text-green-700 dark:bg-green-900/30">
          成功 {stats.succeeded}
        </span>
        <span className="px-2 py-1 rounded bg-blue-100 text-blue-700 dark:bg-blue-900/30">
          运行中 {stats.running}
        </span>
        <span className="px-2 py-1 rounded bg-red-100 text-red-700 dark:bg-red-900/30">
          失败 {stats.failed}
        </span>
        <span className="px-2 py-1 rounded bg-gray-100 text-gray-700 dark:bg-gray-800/60">
          共 {stats.total}
        </span>
        <div className="ml-4 inline-flex rounded border border-gray-300 overflow-hidden">
          <button
            className={`px-3 py-1 ${view === 'dag' ? 'bg-gray-200 dark:bg-gray-700' : ''}`}
            onClick={() => setView('dag')}
          >
            DAG
          </button>
          <button
            className={`px-3 py-1 ${view === 'timeline' ? 'bg-gray-200 dark:bg-gray-700' : ''}`}
