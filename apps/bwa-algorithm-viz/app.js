;(function () {
  ...
  function drawMapqChart() {
    ...
    try {
      els.mapqCanvas.classList.add('mapq-bars-enter')
      setTimeout(() => els.mapqCanvas.classList.remove('mapq-bars-enter'), 700)
    } catch (_) {}
  }
  ...
  function buildModel(ref, read) {
    ...
    const Occ = {}
    for (const c of ABC) {
      Occ[c] = new Array(n + 1).fill(0)
    }
    for (let i = 0; i < n; i++) {
      const ch = L[i]
      for (const c of ABC) {
        Occ[c][i + 1] = Occ[c][i] + (ch === c ? 1 : 0)
      }
    }
  ...
  function findSMEMsForPattern(text, read, minLen) {
    ...
    for (let i = 0; i < rlen; i++) {
      ...
      for (const p0 of arr) {
        let l = 0
        while (i - 1 - l >= 0 && p0 - 1 - l >= 0 && read[i - 1 - l] === text[p0 - 1 - l]) l++
        let r = 0
        while (i + 1 + r < rlen && p0 + 1 + r < tlen && read[i + 1 + r] === text[p0 + 1 + r]) r++
        const readStart = i - l
        const readEnd = i + r + 1
        const refPos = p0 - l
    ...
    for (let i = 0; i < res.length; i++) {
      ...
      for (let j = 0; j < res.length; j++) {
        if (i === j) continue
        const b = res[j]
        if (b.readStart <= a.readStart && b.readEnd >= a.readEnd && b.len > a.len) {
          contained = true
          break
        }
      }
  ...
  function findSMEMs(text, read, minLen, maxOcc, enableRC, enableReseed) {
      ...
      for (const s of agg) {
        ...
        if (occ > (maxOcc || Infinity) || s.len >= 2 * minLen) {
          ...
          if (a.length >= minLen) {
            ...
            if (posa.length) {
              extra.push({ readStart: s.readStart, readEnd: mid, len: a.length, refPosList: posa })
            }
          }
          if (b.length >= minLen) {
            const posb = findOccurrences(text, b, maxOcc || 2000)
            if (posb.length) {
              extra.push({ readStart: mid, readEnd: s.readEnd, len: b.length, refPosList: posb })
            }
          }
        }
      }
    ...
    for (const s of agg) {
      ...
      if (maxOcc && s.refPosList.length > maxOcc) {
        s.refPosList = s.refPosList.slice(0, maxOcc)
      }
    }
    return agg
  }
  ...
  function computeChain(model, smems) {
    ...
    for (let j = 0; j < n; j++) {
      ...
      for (let i = 0; i < j; i++) {
        ...
        const overlap =
          cands[j].readStart < cands[i].readEnd || cands[j].refStart < cands[i].refEnd
            ? state.params.overlapPenalty
            : 0
        const diff = Math.abs(
          cands[j].refStart - cands[i].refStart - (cands[j].readStart - cands[i].readStart)
        )
        if (diff > state.params.diagSlack) continue // 过离散的对角偏移：跳过
        const gapLen = Math.max(0, Math.abs(dr - dq))
        const gapCost =
          (gapLen > 0 ? state.params.gapOpen + state.params.gapExtend * gapLen : 0) +
          2 * diff +
          overlap
    ...
    for (let i = 1; i < n; i++) {
      if (dp[i] > dp[bestIdx]) {
        secondIdx = bestIdx
        bestIdx = i
      } else if (secondIdx === -1 || dp[i] > dp[secondIdx]) {
        secondIdx = i
      }
    }
    const best = []
    ...
    return {
      cands,
      pathIdx: best,
      secondPathIdx: second,
      bestScore: dp[bestIdx] || 0,
      secondScore: secondIdx >= 0 ? dp[secondIdx] : 0,
    }
  }
  ...
  function drawChain(model, chains) {
    ...
    ctx.save()
    ctx.translate(margin - 20 * DPR, margin)
    ctx.rotate(-Math.PI / 2)
    ctx.fillText('read', 0, 0)
    ctx.restore()
    ...
    ctx.strokeStyle = '#6affc1'
    ctx.lineWidth = 2 * DPR
    const tNow = canvas._animTime || performance.now()
    const dur = 1200
    const t0 = canvas._chainStartTime || tNow
    if (!canvas._chainStartTime) canvas._chainStartTime = tNow
    ...
    if (chains.secondPathIdx && chains.secondPathIdx.length) {
      ...
      const dur2 = 900
      const t02 = canvas._chainStartTime2 || tNow2
      if (!canvas._chainStartTime2) canvas._chainStartTime2 = tNow2
  ...
  function simulateBandedSW(model, chains) {
    if (!chains || !chains.pathIdx || chains.pathIdx.length === 0)
      return { w: state.params.swBand, Z: state.params.zdrop, best: 0, drop: false }
    const first = chains.cands[chains.pathIdx[0]]
    const refStart = Math.max(0, first.refStart - 5)
    const w = state.params.swBand,
      Z = state.params.zdrop
    let best = 0,
      cur = 0,
      drop = false
  ...
  function approxMAPQ(chains, smems) {
    ...
    let occSum = 0
    for (const s of smems || []) occSum += Math.max(0, (s.refPosList?.length || 0) - 1)
  ...
  function renderSATable(model, revealCount) {
    ...
    rotTitle.className = 'rotation'
    rotTitle.style.color = 'var(--muted)'
    rotTitle.textContent = '排序后的旋转'
    ...
    for (let i = 0; i < n; i++) {
      ...
      rot.className = 'rotation'
      for (const ch of model.sortedRot[i]) rot.appendChild(makeCell(ch))
      if (i < revealCount) {
        rot.classList.add('pulse')
      }
      row.appendChild(rot)
      const sa = document.createElement('div')
      sa.className = 'sa'
      sa.textContent = String(model.SA[i])
      row.appendChild(sa)
  ...
  function renderBWTPanel(model, showFL, activeRange) {
    ...
    const makeBlock = (title, arr) => {
      ...
      wrap.className = 'bwt-block'
      const t = document.createElement('div')
      t.className = 'bwt-title'
      t.textContent = title
      wrap.appendChild(t)
      const col = document.createElement('div')
      col.className = 'bwt-col'
      col.style.flexDirection = 'column'
      arr.forEach((ch, i) => {
        const cell = makeCell(ch)
        if (activeRange && i >= activeRange.l && i < activeRange.r) {
  ...
  function renderOccTable(model, show) {
    ...
    idxh.className = 'idx'
    idxh.textContent = 'i'
    header.appendChild(idxh)
    for (const c of ABC) {
      header.appendChild(makeCell(c))
    }
    ...
    for (let i = 0; i <= limit; i++) {
      ...
      idc.className = 'idx'
      idc.textContent = String(i)
      row.appendChild(idc)
      for (const c of ABC) {
        row.appendChild(makeCell(String(model.Occ[c][i])))
      }
      els.occTable.appendChild(row)
    }
    if (model.n > 200) {
      const more = document.createElement('div')
      more.className = 'table-row'
      more.appendChild(makeCell('...'))
      els.occTable.appendChild(more)
    }
  }
    ...
    if (state.stepIndex < state.steps) {
      ...
      try {
        ...
        const activeIdx = Math.max(
          0,
          Math.min(rows.length - 1, state.stepIndex - state.segments.search.start)
        )
  ...
  function onGenerate() {
    try {
      ...
      state.smems = findSMEMs(
        model.text,
        model.pattern,
        state.params.minSeed,
        state.params.maxOcc,
        state.params.enableRC,
        state.params.enableReseed
      )
      renderSMEM(model, state.smems)
      state.chains = computeChain(model, state.smems)
      drawChain(model, state.chains)
      const sw = simulateBandedSW(model, state.chains)
      renderSW(sw)
      drawSWMatrix(model, state.chains, sw)
      state.mapq = approxMAPQ(state.chains, state.smems)
      renderMAPQ(state.mapq)
    } catch (e) {
      log('错误：' + e.message)
  ...
  if (els.applyParamsBtn) {
    els.applyParamsBtn.addEventListener('click', applyParams)
  }
  ...
  if (exportChainBtn && els.chainCanvas) {
    exportChainBtn.addEventListener('click', () => exportCanvasPNG(els.chainCanvas, 'chain.png'))
  }
  const exportSWBtn = document.getElementById('exportSWBtn')
  if (exportSWBtn && els.swCanvas) {
    exportSWBtn.addEventListener('click', () => exportCanvasPNG(els.swCanvas, 'sw.png'))
  }
  ...
  if (els.chainCanvas && chainTooltip) {
    els.chainCanvas.addEventListener('mousemove', (ev) => {
      ...
      const cache = els.chainCanvas._chainCache
      if (!cache) {
    ...
    els.chainCanvas.addEventListener('mouseleave', () => {
      chainTooltip.style.display = 'none'
    })
  }
  ...
  function drawSWMatrix(model, chains, swStat) {
    ...
    const n = maxPat,
      m = maxRef // rows (pattern), cols (ref)
    if (n <= 0 || m <= 0) {
      return
    }
    const cellSize = Math.max(
      6 * DPR,
      Math.min(14 * DPR, Math.min((W - margin * 2) / m, (H - margin * 2) / n))
    )
    const ox = margin,
      oy = margin
    // DP arrays (banded)
    ...
    // grid lines (sparse)
    ctx.strokeStyle = 'rgba(42,59,102,0.8)'
    ctx.lineWidth = 1
    for (let i = 0; i <= n; i++) {
      const y = oy + i * cellSize
      ctx.beginPath()
      ctx.moveTo(ox, y)
      ctx.lineTo(ox + m * cellSize, y)
      ctx.stroke()
    }
    for (let j = 0; j <= m; j++) {
    ...
    if (path.length) {
      ...
      const tNow = canvas._animTime || performance.now()
      const dur = 1000
      const t0 = canvas._swStartTime || tNow
      if (!canvas._swStartTime) canvas._swStartTime = tNow
      const p = Math.min(1, (tNow - t0) / dur)
      const nDraw = Math.max(1, Math.floor(path.length * p))
      ctx.beginPath()
      for (let k = 0; k < nDraw; k++) {
        const [i, j] = path[path.length - 1 - k]
        const cx = ox + (j - 0.5) * cellSize
        const cy = oy + (i - 0.5) * cellSize
        if (k === 0) ctx.moveTo(cx, cy)
        else ctx.lineTo(cx, cy)
      }
      ctx.stroke()
