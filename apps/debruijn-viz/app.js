;(function () {
  ...
  function build() {
    state.text = sanitizeDNA(els.refSeq.value)
    state.k = Math.max(2, Math.min(10, parseInt(els.kVal.value || '3')))
    const kmers = []
    for (let i = 0; i + state.k <= state.text.length; i++) {
      kmers.push(state.text.slice(i, i + state.k))
    }
    const nodesMap = new Map()
    const edges = []
    for (const kmer of kmers) {
      const u = kmer.slice(0, state.k - 1),
        v = kmer.slice(1)
      if (!nodesMap.has(u)) nodesMap.set(u, { id: u })
      if (!nodesMap.has(v)) nodesMap.set(v, { id: v })
      edges.push({ u, v, kmer })
    }
    ...
    for (const e of edges) {
      degOut.set(e.u, (degOut.get(e.u) || 0) + 1)
      degIn.set(e.v, (degIn.get(e.v) || 0) + 1)
    }
    ...
    let start = state.nodes[0]?.id
    for (const n of state.nodes) {
      const o = degOut.get(n.id) || 0,
        i = degIn.get(n.id) || 0
      if (o > i) {
        start = n.id
        break
  ...
  function draw(progress) {
    ...
    for (let i = 0; i < state.nodes.length; i++) {
      ...
      const x = cx + R * Math.cos(a),
        y = cy + R * Math.sin(a)
      pos.set(state.nodes[i].id, { x, y })
    }
    // edges
    ctx.strokeStyle = '#2a3b66'
    ctx.lineWidth = 1
    for (const e of state.edges) {
      const p1 = pos.get(e.u),
        p2 = pos.get(e.v)
      if (!p1 || !p2) continue
      ctx.beginPath()
    ...
    if (state.euler && state.euler.length > 1) {
      const max = Math.floor(Math.min(progress, state.euler.length - 1))
      ctx.strokeStyle = '#6affc1'
      ctx.lineWidth = 3
      ctx.beginPath()
      for (let i = 0; i < max; i++) {
        const a = state.euler[i],
          b = state.euler[i + 1]
        const p1 = pos.get(a),
          p2 = pos.get(b)
        if (i === 0) {
