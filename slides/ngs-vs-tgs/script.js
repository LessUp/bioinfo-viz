;(function () {
    ...
    try {
      ...
      if (typeof s.notesVisible === 'boolean') {
        ...
        document
          ...
          .forEach((n) => n.classList.toggle('hidden', !notesVisible))
      }
  ...
  function revealStep(step) {
    ...
    frags.forEach((f, i) => {
      if (i < currentStep) f.classList.add('visible')
  ...
  function updatePresenter() {
    ...
    if (presenterNext)
      presenterNext.innerHTML = nextSlide
        ? cloneForPresenter(nextSlide)
        : '<div class="text-slate-500">已到最后一页</div>'
    if (presenterNotes) {
  ...
  if (presenterOverlay)
    presenterOverlay.addEventListener('click', (e) => {
  ...
  let initPage =
    parsed.pageIndex != null
      ? parsed.pageIndex
      : state && typeof state.page === 'number'
        ? state.page
        : 0
  let initStep =
    parsed.step != null ? parsed.step : state && typeof state.step === 'number' ? state.step : 0
  goTo(initPage)
